...
pub fn svg_to_bitmap(
    ...
    let mut pixmap =
        tiny_skia::Pixmap::new(target_width, target_height).context("无法创建像素图")?;
    ...
    if let Some(bg_color) = options.background_color {
        ...
        let rect = tiny_skia::Rect::from_xywh(0.0, 0.0, target_width as f32, target_height as f32)
            .unwrap();
...
fn main() {
    ...
    match svg_to_bitmap(svg_data, &options) {
        Ok((bitmap_data, width, height)) => {
            ...
            let bmp =
                image::ImageBuffer::<image::Rgba<u8>, _>::from_raw(width, height, bitmap_data)
                    .unwrap();
